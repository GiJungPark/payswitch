...
기본적으로 production code 구현과 그 코드에 대한 리뷰 수정은 Claude Code에 맡긴다. 사용자가 Codex에게 직접 구현을 요청하거나 Claude가 진행할 수 없는 상황에서 사용자가 명시적으로 허용한 경우에만 Codex가 production code를 작성한다. Codex는 리뷰 결과와 재현 방법을 Claude에게 전달하고, 저장소 운영 문서, Issue/PR 문안과 ADR 수정을 담당한다.

Claude는 완료 보고를 현재 Claude Code 세션에 출력한다. Codex는 해당 terminal output과 실제 diff를 수집해 리뷰하며 Claude가 Codex에 직접 메시지를 보낸다고 가정하지 않는다.
...
사용자의 새 지시가 구현 범위나 설계를 바꾸면 Codex가 Issue 또는 ADR을 갱신하고 새 구현 지시서를 만든 뒤에만 Claude에 전달한다. 자연어 요청만으로 Claude의 기존 범위나 역할 제한을 우회하지 않는다.
...
7. 아래 형식으로 Claude Code에 구현 지시서를 작성한다.
...
같은 Issue와 branch에서 Claude의 구현을 수정시키는 follow-up도 새 Claude 세션만으로 검증할 수 있게 작성한다.
...
문제가 있으면 Codex가 production code를 대신 고치기보다 위 리뷰 수정 지시서로 구체적인 리뷰 항목과 재현 방법을 Claude에게 전달해 수정하게 한다. 단, 아래 리뷰 예산과 종료 조건을 적용한다.
...
4. 반영할 production code finding은 한 번의 Claude follow-up으로 통합한다. Codex 소유 문서 변경은 Codex가 처리한다.
...
- 모든 Issue와 구현 지시서에 Documentation impact를 작성한다. 동작·contract·구조·운영 방법이 바뀌면 가장 가까운 README와 상세 문서를 같은 작업에서 갱신한다.
